...
This directory contains knowledge base articles for **humans and GenAI agents** to understand and use the Pod-OS Python client (`pod_os_client`). The content is aligned with the Pod-OS Neural Memory and messaging model and uses **Python** examples throughout.
...
| [docs/Pod-OS-Neural-Memory-Event-Prompts.md](docs/Pod-OS-Neural-Memory-Event-Prompts.md) | Neural Memory events, tags, links; storing events/tags/links with Python examples |
...
- **AI agents**: Use these docs as context when answering questions or generating code that uses the Pod-OS Python client (e.g. `Client`, `Config`, `Message`, `EventFields`, intents, Neural Memory operations).
